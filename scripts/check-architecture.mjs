...
const forbiddenTopLevelBaskets = ['services', 'types', 'contexts', 'lib', 'hooks', 'components'];
...
for (const basket of forbiddenTopLevelBaskets) {
  ...
  if (fs.existsSync(dir)) {
    fail(dir, `top-level ${basket}/ basket is forbidden; place code in features/, infrastructure/, providers/, shared/, config/, or app/`);
  }
}
  ...
  if (featureMatch) {
    ...
    for (const specifier of imports) {
      ...
      if (importedFeature !== currentFeature && suffix !== '/public') {
        fail(file, `cross-feature import must use @/features/${importedFeature}/public, not ${specifier}`);
      }
    }
  }
  ...
  if (relative.startsWith('src/providers/')) {
    for (const specifier of imports) {
      ...
      if (imported && imported[2] !== '/public') {
        fail(file, `provider may consume a feature only through @/features/${imported[1]}/public`);
      }
    }
  }

  if ((relative.startsWith('src/features/') || relative.startsWith('src/providers/') || relative.startsWith('src/infrastructure/')) && /queryKey\s*:\s*\[/.test(source)) {
    fail(file, 'ad-hoc TanStack query keys are forbidden; use the owning feature query-key factory');
  }

  if (/^src\/features\/[^/]+\/screens\/.+\.tsx$/.test(relative)) {
    if (/\buseQuery\s*\(/.test(source) || /\buseMutation\s*\(/.test(source)) {
      fail(file, 'screens cannot own TanStack queries/mutations; move server-state orchestration to queries/, mutations/, or hooks/');
    }
    if (/from\s+['"]@\/features\/[^'"]+\/api\//.test(source) || /from\s+['"]\.\.\/api\//.test(source)) {
      fail(file, 'screens cannot import API adapters directly; consume feature hooks/use-cases instead');
    }
  }

  if (relative.startsWith('src/app/') && relative.endsWith('.tsx')) {
    const normalized = relative.slice('src/app/'.length);
    const compositionFiles = new Set(['_layout.tsx', 'index.tsx', '(tabs)/_layout.tsx']);
    if (!compositionFiles.has(normalized)) {
      const compact = source.trim();
      const routeWrapper = /^export \{ default \} from ['"]@\/features\/[^'"]+\/screens\/[^'"]+['"];?$/.test(compact);
      if (!routeWrapper) {
        fail(file, 'route files must be thin one-line feature screen re-exports; business/UI logic belongs in features/');
      }
    }
  }
}
