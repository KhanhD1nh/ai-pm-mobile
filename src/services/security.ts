...
export async function setBiometricLockEnabled(enabled: boolean) {
  if (enabled) {
    ...
    if (!compatible || !enrolled) throw new Error('Thiết bị chưa cấu hình sinh trắc học');
    const result = await LocalAuthentication.authenticateAsync({ promptMessage: 'Bật khóa sinh trắc học cho AI-PM', cancelLabel: 'Hủy' });
