  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      const invalidateIssueEvent = (rawEvent: unknown) => {
        ...
        const projectId =
          payload?.project_id ?? payload?.data?.issue?.project_id ?? null;
        const identifier =
          payload?.data?.issue?.identifier ?? payload?.data?.identifier ?? null;
      ...
      for (const event of ["cycle.updated", "milestone.updated"]) {
        source.addEventListener(event as any, (rawEvent: unknown) => {
          ...
          try {
            ...
            projectId =
              payload?.project_id ?? payload?.data?.project_id ?? null;
          } catch {
