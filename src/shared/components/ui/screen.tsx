...
export function Screen({
  ...
  const content = <FocusTransitionView style={[styles.content, !scroll && styles.contentFill]}>{children}</FocusTransitionView>;
        ...
        isStack ? (
          <View style={styles.stackHeaderOuter}>
            <View style={styles.stackHeader}>
              <HeaderBackButton kind={chrome === 'modal' ? 'close' : 'back'} onPress={handleBack} />
...
export function ErrorState({ title, body, onRetry }: { title: string; body?: string; onRetry?: () => void }) {
  ...
  return (
    <View accessibilityRole="alert" style={styles.empty}>
      <View style={styles.emptyIcon}><Ionicons accessible={false} name="alert-circle-outline" size={22} color={theme.colors.danger} /></View>
      <Text style={styles.emptyTitle}>{title}</Text>
      {body ? <Text style={styles.emptyBody}>{body}</Text> : null}
      {onRetry ? (
        <MotionPressable accessibilityRole="button" onPress={onRetry} style={styles.retryButton}>
          <Text style={styles.retryText}>{language === 'vi' ? 'Thử lại' : 'Try again'}</Text>
        </MotionPressable>
      ) : null}
    </View>
  );
...
const createStyles = (ui: AppTheme) => StyleSheet.create({
  ...
  stackHeaderOuter: { width: '100%', zIndex: 20, paddingHorizontal: ui.header.horizontalInset, paddingVertical: ui.header.verticalInset },
  stackHeader: { width: '100%', maxWidth: 820, minHeight: ui.header.actionSize, alignSelf: 'center', flexDirection: 'row', alignItems: 'center' },
  ...
  stackRight: { minWidth: ui.header.actionSize, minHeight: ui.header.actionSize, alignItems: 'flex-end', justifyContent: 'center' },
  ...
  retryButton: { minHeight: 44, paddingHorizontal: 16, borderRadius: ui.radius.md, alignItems: 'center', justifyContent: 'center', backgroundColor: ui.colors.accentSoft, marginTop: 4 },
