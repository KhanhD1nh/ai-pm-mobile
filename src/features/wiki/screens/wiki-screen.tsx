...
import { BottomSheet, GlassIconButton, ListGroup, SectionHeader } from '@/shared/components/ui/mobile';
...
export default function WikiScreen() {
  ...
  return (
    <Screen
      ...
      right={selected ? <GlassIconButton icon="create-outline" label={language === 'vi' ? 'Sửa trang' : 'Edit page'} onPress={() => startEdit(selected)} /> : undefined}
    >
      {selected ? (
        <>
          ...
          <SectionHeader
            ...
            right={(
              <MotionPressable
                ...
                accessibilityLabel={language === 'vi' ? 'Tạo trang mới' : 'Create new page'}
                onPress={startNew}
                style={styles.newPageAction}
              >
                <Ionicons accessible={false} name="add" size={18} color={ui.colors.accentStrong} />
                <Text style={styles.newPageActionText}>{language === 'vi' ? 'Trang mới' : 'New page'}</Text>
              </MotionPressable>
            )}
          />
...
const createStyles = (ui: AppTheme) => StyleSheet.create({
  newPageAction: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 4 },
  newPageActionText: { color: ui.colors.accentStrong, ...ui.typography.bodyStrong },
