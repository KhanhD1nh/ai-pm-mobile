...
export default function PlanningScreen() {
  ...
  const { project, cycles, milestones, scheduled, refresh } =
  ...
  const scheduledForDay = useMemo(
    () =>
      scheduled
        .filter(
          (issue) =>
            issue.scheduled_start &&
            startOfDay(new Date(issue.scheduled_start)).getTime() ===
  ...
  const submit = () => {
    ...
    if (mode === "cycle") {
      createCycle.mutate(
        { name: name.trim(), startsAt: start, endsAt: end },
        { onSuccess, onError },
      );
      return;
    }
    createMilestone.mutate(
      { title: name.trim(), startDate: start || undefined, targetDate: end },
      { onSuccess, onError },
    );
  };
  ...
  return (
    ...
    >
        ...
        {days.map((date, index) => {
          ...
          return (
            <MotionPressable
              ...
              onPress={() => setSelectedOffset(index)}
            ...
            >
              ...
              <View
                style={[styles.eventDot, hasEvent && styles.eventDotVisible]}
              />
            </MotionPressable>
          );
        })}
        ...
        {scheduledForDay.length === 0 ? (
          <View style={styles.emptySchedule}>
            ...
            <Text style={styles.emptyTitle}>
              {language === "vi" ? "Chưa có lịch" : "Nothing scheduled"}
            </Text>
            <Text style={styles.emptyBody}>
              {language === "vi"
                ? "Các focus session trong ngày sẽ xuất hiện tại đây."
                : "Focus sessions for the day will appear here."}
            </Text>
          </View>
        ) : (
          scheduledForDay.map((issue, index) => {
            ...
            const ends = new Date(
              starts.getTime() +
                Number(issue.focus_hours ?? 1) * 60 * 60 * 1000,
            );
            return (
              <View key={issue.id} style={styles.timelineRow}>
                ...
                <View style={styles.lineColumn}>
                  ...
                  {index < scheduledForDay.length - 1 ? (
                ...
                >
                  <Pill
                    text={`${starts.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })} – ${ends.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })}`}
                    tone="accent"
                  />
                  ...
                  <View style={styles.eventMeta}>
                    <Text style={styles.eventIdentifier}>
                      {issue.identifier}
                    </Text>
                    {issue.focus_hours ? (
                      <Text style={styles.eventIdentifier}>
                        {Number(issue.focus_hours)}h focus
                      </Text>
                    ) : null}
                  </View>
                </MotionPressable>
      ...
      <SectionHeader
        title={language === "vi" ? "Kế hoạch dự án" : "Project planning"}
        caption={
          language === "vi" ? "Cycle và milestone" : "Cycles and milestones"
        }
      />
      <View style={styles.planSummary}>
        <MotionPressable
          accessibilityRole="button"
          onPress={() => setMode("cycle")}
          style={[styles.planCard, styles.planCardDivider]}
        >
          <Ionicons
            accessible={false}
            name="repeat-outline"
            ...
            color={ui.colors.accentStrong}
          />
          <Text style={styles.planValue}>{cycles.data?.length ?? 0}</Text>
          <Text style={styles.planLabel}>Cycles</Text>
        </MotionPressable>
        <MotionPressable
          accessibilityRole="button"
        ...
        >
          ...
          />
          <Text style={styles.planValue}>{milestones.data?.length ?? 0}</Text>
      ...
      <BottomSheet
        ...
        footer={
          <Button
            title={language === "vi" ? "Tạo" : "Create"}
            disabled={!name.trim() || !end || (mode === "cycle" && !start)}
            onPress={submit}
          />
        }
      >
        ...
        />
        <Field
          placeholder="Start: 2026-09-04"
          value={start}
          onChangeText={setStart}
        />
        <Field
          placeholder={
            mode === "cycle" ? "End: 2026-09-18" : "Target: 2026-09-18"
          }
          value={end}
          onChangeText={setEnd}
        />
