...
export default function BoardScreen() {
  ...
  const sections = useMemo(() => {
    const grouped = new Map(
      orderedStatuses.map((status) => [status.id, [] as typeof issueItems]),
    );
    ...
    create.mutate(
      {
        ...
        priority,
        statusId: createStatusId || orderedStatuses[0]?.id,
  ...
  return (
    <Screen edges={[]} scroll={false}>
      <SectionList
        ...
        onEndReached={() => {
          if (issues.hasNextPage && !issues.isFetchingNextPage)
          ...
          <View style={styles.toolbar}>
            ...
            <View style={styles.filterLine}>
              <MotionPressable
                ...
                accessibilityLabel={
                  language === "vi" ? "Mở bộ lọc" : "Open filters"
                }
        ...
        ListEmptyComponent={
          hasNarrowing ? (
            ...
            <EmptyState
              title={language === "vi" ? "Chưa có công việc" : "No tasks yet"}
            />
          )
        }
        renderSectionHeader={({ section }) => (
          <View style={styles.statusHeader}>
            ...
            <MotionPressable
              accessibilityRole="button"
              accessibilityLabel={`${language === "vi" ? "Thêm công việc vào" : "Add task to"} ${section.status.name}`}
              onPress={() => openCreateFor(section.status.id)}
      ...
      <BottomSheet
        ...
        subtitle={
          language === "vi"
            ? "Chỉ giữ lại công việc bạn cần lúc này"
            : "Keep only the work you need right now"
        }
      ...
      >
        ...
        {priorities.map((item) => (
          <ChoiceRow
            key={item}
            label={
              item === "ALL" ? (language === "vi" ? "Tất cả" : "All") : item
            }
        ...
        {(members.data ?? []).map((member) => (
          <ChoiceRow
            ...
            onPress={() => setAssigneeFilter(member.id)}
          />
        ))}
        <Text style={styles.sheetLabel}>Cycle</Text>
        <ChoiceRow
          label={language === "vi" ? "Tất cả" : "All"}
          active={cycleFilter === "ALL"}
          onPress={() => setCycleFilter("ALL")}
        />
        <ChoiceRow
          label={language === "vi" ? "Không có cycle" : "No cycle"}
          active={cycleFilter === "NONE"}
          onPress={() => setCycleFilter("NONE")}
        />
        {(cycles.data ?? []).map((cycle) => (
          <ChoiceRow
            ...
            onPress={() => setCycleFilter(cycle.id)}
          />
        ))}
      </BottomSheet>
      ...
      <BottomSheet
        visible={openCreate}
        title={language === "vi" ? "Tạo công việc" : "Create task"}
        subtitle={
          orderedStatuses.find((status) => status.id === createStatusId)
      ...
      >
        <Field
          placeholder={language === "vi" ? "Tiêu đề công việc" : "Task title"}
          value={title}
        ...
        {priorities
          ...
          .map((item) => (
            <ChoiceRow
              ...
              onPress={() => setPriority(item as Priority)}
            />
          ))}
        <Text style={styles.sheetLabel}>Assignee</Text>
        <ChoiceRow
          label={language === "vi" ? "Chưa giao" : "Unassigned"}
          active={!createAssignee}
        ...
        {(members.data ?? []).map((member) => (
          <ChoiceRow
            key={member.id}
            label={member.name}
            active={createAssignee === member.id}
            onPress={() => setCreateAssignee(member.id)}
          />
        ))}
      </BottomSheet>
    </Screen>
