...
export function useAddOrganizationMember(orgId?: string | null) {
  ...
  return useMutation({
    mutationFn: ({ email, role }: { email: string; role: string }) => organizationsApi.addMember(orgId!, email, role),
...
export function useUpdateOrganizationMember(orgId?: string | null) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, role }: { id: string; role: string }) => organizationsApi.updateMember(orgId!, id, role),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: organizationKeys.members(orgId) }),
  });
}
...
export function useUpdateAiBudget(orgId?: string | null) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: Partial<Pick<AiBudget, 'monthlyUsdLimit' | 'alertThresholdPct' | 'resetDay'>>) => organizationsApi.updateAiBudget(orgId!, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: organizationKeys.budget(orgId) }),
  });
}
