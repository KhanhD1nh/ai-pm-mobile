...
type BudgetInput = Pick<
  AiBudget,
  "monthlyUsdLimit" | "alertThresholdPct" | "resetDay"
>;
...
export default function AiBudgetScreen() {
  ...
  if (budget.isLoading)
    return (
      <LoadingScreen
        ...
        subtitle={
          language === "vi"
            ? "Kiểm soát chi phí AI của workspace"
            : "Workspace AI cost governance"
        }
      />
    );
  ...
  const pct =
    monthly > 0 ? Math.min(100, Math.round((usage / monthly) * 100)) : 0;
    ...
    >
      ...
      >
        ...
        >
          <View>
            <Text
              style={{ color: ui.colors.textMuted, ...ui.typography.caption }}
            >
              {language === "vi" ? "Đã dùng tháng này" : "Used this month"}
            </Text>
            <Text
              style={{
        ...
        >
          <View
            style={{
              ...
              borderRadius: 4,
              backgroundColor:
                pct >= (budget.data?.alertThresholdPct ?? 80)
                  ? ui.colors.warning
                  : ui.colors.accent,
            }}
          />
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 7 }}>
          ...
          />
          <Text
            style={{ color: ui.colors.textSecondary, ...ui.typography.body }}
          >
            {pct}%{" "}
            {language === "vi"
              ? "ngân sách tháng đã sử dụng"
      ...
      {budget.data ? (
        ...
        <Text style={{ color: ui.colors.textMuted }}>
          {language === "vi"
            ? "Chưa có cấu hình AI Budget."
            : "No AI Budget configured."}
        </Text>
      )}

      <SectionHeader
        title={language === "vi" ? "Cách hoạt động" : "How it works"}
      />
      <ListGroup>
        <ListRow
          first
          icon="notifications-outline"
          label={language === "vi" ? "Ngưỡng cảnh báo" : "Alert threshold"}
          value={`${budget.data?.alertThresholdPct ?? 80}%`}
          detail={
            language === "vi"
              ? "Cảnh báo khi usage chạm ngưỡng này."
              : "Warn when usage reaches this level."
          }
        />
        <ListRow
          icon="refresh-outline"
          label={language === "vi" ? "Ngày reset" : "Reset day"}
          value={`${budget.data?.resetDay ?? 1}`}
          detail={
            language === "vi"
              ? "Ngày bắt đầu chu kỳ ngân sách mới."
              : "Day the monthly budget cycle restarts."
          }
        />
      </ListGroup>
    </Screen>
...
function BudgetForm({
  ...
  return (
    <>
      <SectionHeader
        title={language === "vi" ? "Thiết lập ngân sách" : "Budget settings"}
      />
      <View style={{ gap: 10 }}>
        <Field
          keyboardType="decimal-pad"
          placeholder={
            language === "vi" ? "Giới hạn tháng (USD)" : "Monthly limit (USD)"
          }
          value={limit}
        ...
        <Field
          ...
          placeholder={
            language === "vi" ? "Ngưỡng cảnh báo (%)" : "Alert threshold (%)"
          }
          value={threshold}
        ...
        <Field
          keyboardType="number-pad"
          placeholder={language === "vi" ? "Ngày reset" : "Reset day"}
          value={resetDay}
        ...
        <Button
          ...
          onPress={() =>
            onSubmit({
              monthlyUsdLimit: parsedLimit,
              alertThresholdPct: Number(threshold),
