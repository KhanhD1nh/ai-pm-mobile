export function resolveExternalPath(rawUrl: string): string | null {
  try {
    ...
    if (host === "issue" && parts[0])
      return `/(tabs)/(home)/issue/${encodeURIComponent(parts[0])}`;
    if (host === "project" && parts[0])
      return `/(tabs)/(projects)/project/${encodeURIComponent(parts[0])}`;
    if (parts[0] === "issue" && parts[1])
      return `/(tabs)/(home)/issue/${encodeURIComponent(parts[1])}`;
    if (parts[0] === "project" && parts[1])
      return `/(tabs)/(projects)/project/${encodeURIComponent(parts[1])}`;
    return null;
  } catch {
    return null;
  }
