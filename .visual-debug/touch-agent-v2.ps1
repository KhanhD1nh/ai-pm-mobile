...
$queue='E:\WorkSpace\AI-PM\ai-pm-mobile\.visual-debug\touch-agent-v2.cmd'
$ack='E:\WorkSpace\AI-PM\ai-pm-mobile\.visual-debug\touch-agent-v2.ack'
$last=''
while ($true) {
  if (Test-Path $queue) {
    $line=(Get-Content $queue -Raw).Trim()
    if ($line -and $line -ne $last) {
      $parts=$line.Split('|'); $seq=$parts[0]
      try {
        if ($parts[1] -eq 'tap') {
