...
For the sideload artifact produced by GitHub CI, AI-PM runs `ipapatch` v2.1.3 with a pinned Spotify-style `zxPluginsInject` compatibility shim. This variant mirrors `group.*` `NSUserDefaults` writes from the host app into the shared container and redirects app-extension reads to that container. That fan-out matters for `expo-widgets`, which stores serialized widget layouts and timelines in App Group `UserDefaults`. The shim is injected into both the main executable and every embedded app extension. CI verifies that both the host app and widget extension load `@rpath/zxPluginsInject.dylib` before uploading `AI-PM-sideload-patched.ipa`. Use that patched IPA for ESign-style sideload tests; keep `AI-PM-unsigned.ipa` as the unmodified reference build.
